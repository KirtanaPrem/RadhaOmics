...
.ro-nav {
    ...
    padding: 14px 2rem; border-bottom: 0.5px solid #e5e7eb;
...
.ro-sub { font-size: 13px; color: #666; font-weight: 300; line-height: 1.7; margin-bottom: 1.5rem; }
.sl { font-size: 9px; letter-spacing: 0.16em; text-transform: uppercase; color: #bbb; margin-bottom: 8px; }
...
.side-gene { font-size: 11px; color: #1B6CA8; padding: 4px 0; border-bottom: 0.5px solid #f5f5f5; font-family: 'IBM Plex Mono', monospace; }
...
.section-label { font-size: 9px; font-weight: 500; letter-spacing: 0.14em; text-transform: uppercase; color: #bbb; margin-bottom: 10px; margin-top: 1.5rem; }
...
.deg-table { width: 100%; border-collapse: collapse; font-size: 11px; }
...
.ro-footer { border-top: 0.5px solid #e5e7eb; padding: 12px 24px; text-align: center; font-size: 10px; color: #ccc; font-family: 'Instrument Serif', serif; font-style: italic; margin-top: 2rem; }
...
with left:
    ...
    <div style="margin-top:1.5rem;">
      <div class="sl">GTEx · {tissue}</div>
      <div class="side-big">{pct}%</div>
      <div class="side-body" style="margin-bottom:3px;">of genes sex-biased</div>
      <div class="bar-wrap"><div class="bar-fill" style="width:{pct}%;"></div></div>
      <div class="side-body" style="margin-bottom:1rem;">{info['note']}</div>
      {''.join(f'<div class="side-gene">{g}</div>' for g in info['top_genes'])}
      <div class="ro-divider"></div>
      <div class="sl">Tissue weight</div>
...
with main:
    ...
    if not load_example and uploaded_file is None:
        ...
        for i, (title, desc) in enumerate(features):
            ...
            with col:
                st.markdown(f'<div style="border:0.5px solid #e5e7eb;border-radius:10px;padding:1rem 1.25rem;margin-bottom:8px;"><div class="sl">{title}</div><div style="font-size:12px;color:#666;line-height:1.6;">{desc}</div></div>', unsafe_allow_html=True)
    ...
    if load_example or uploaded_file is not None:
        ...
        st.markdown('<div class="section-label" style="margin-top:0;">Sample distribution & compliance</div>', unsafe_allow_html=True)
        ...
        with dc1:
            ...
            fig.update_layout(
                ...
                annotations=[dict(
                    text=f"<b>{ratio['total']}</b><br><span style='font-size:9px'>samples</span>",
        ...
        # DEG Impact
        st.markdown('<div class="section-label">Differential expression impact</div>', unsafe_allow_html=True)
        st.markdown("""
        ...
        <div style="font-size:10px;color:#888;margin-top:6px;margin-bottom:1rem;">3 of 5 top DEGs would lose significance in a sex-balanced cohort</div>
        """, unsafe_allow_html=True)

        # BiasΔ breakdown
        st.markdown('<div class="section-label">BiasΔ propagation breakdown</div>', unsafe_allow_html=True)
        ...
        # Gene flags
        if flags:
            st.markdown('<div class="section-label">Sex-biased genes detected</div>', unsafe_allow_html=True)
        ...
        # Recommendations
        st.markdown("""
        <div class="section-label">Corrective recommendations</div>
        ...
        st.markdown('<div class="section-label">Ready for submission?</div>', unsafe_allow_html=True)
        ...
        <div class="sub-row"><div class="sub-check {'sub-yes' if not flags else 'sub-no'}">{'✓' if not flags else '✕'}</div><span>Sex-biased genes excluded from normalisation</span></div>
        ...
        citation = (
            ...
            f"Does not meet NIH SABV or FDA sex-disaggregated reporting standards."
        )
        st.markdown('<div class="section-label">Methods citation</div>', unsafe_allow_html=True)
...
with right:
    ...
    st.markdown('<div class="sl">Where you land</div>', unsafe_allow_html=True)
    for low, high, color, label, desc in [
        ...
        if score is not None and low <= score <= high:
            marker = f'<div class="band-marker" style="color:{color};">◀ your dataset</div>'
        st.markdown(f"""
    ...
    st.markdown('<div class="sl">In context</div>', unsafe_allow_html=True)
    if score:
    ...
    st.markdown('<div class="ro-divider"></div>', unsafe_allow_html=True)
    st.markdown('<div class="sl">What reviewers will ask</div>', unsafe_allow_html=True)
    ...
    if results:
        ...
        f = results["flags"]
        st.markdown(f"""
        <div class="reviewer-q">
          <div class="reviewer-q-text">"What is your male to female sample ratio?"</div>
          <div class="reviewer-q-ans">{r['mf_ratio']}:1 — must disclose</div>
        </div>
        <div class="reviewer-q">
          <div class="reviewer-q-text">"Did you account for sex as a biological variable?"</div>
          <div class="reviewer-q-ans">NIH SABV {'not met' if r['ratio_flag'] else 'met'}</div>
        </div>
        <div class="reviewer-q">
          <div class="reviewer-q-text">"Are your DEGs reproducible in a balanced cohort?"</div>
          <div class="reviewer-q-ans">3 of 5 top DEGs at risk</div>
        </div>
        <div class="reviewer-q">
          <div class="reviewer-q-text">"Is female sample size sufficient?"</div>
          <div class="reviewer-q-ans">Power {round(p['achieved_power']*100)}% — {'insufficient' if p['power_flag'] else 'sufficient'}</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
    ...
    st.markdown("""
    <div style="margin-top:auto;padding-top:3rem;font-size:10px;line-height:2.2;">
