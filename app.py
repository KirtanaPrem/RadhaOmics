...
from radhaomics import (
    ...
    compute_real_degs, compute_field_percentile,
    compute_samples_needed, generate_dynamic_recommendations,
)

st.set_page_config(page_title="RadhaOmics", layout="wide",
...
def fmt_p(p):
    if p < 0.001:
        exp = int(np.floor(np.log10(abs(p) + 1e-300)))
        base = p / (10 ** exp)
        return f"{base:.1f}×10{superscript(exp)}"
    return f"{p:.3f}"
...
section[data-testid="stSidebar"] { background: #fafaf8 !important; border-right: 0.5px solid #e5e7eb !important; }
section[data-testid="stSidebar"] > div { padding: 1.5rem 1.25rem !important; }
.ro-pill { display: inline-flex; align-items: center; gap: 5px; font-size: 11px; color: #1B6CA8; background: #EBF3FA; padding: 4px 12px; border-radius: 20px; margin-bottom: 1rem; }
...
.cn { font-size: 9px; padding: 1px 5px; border-radius: 3px; background:#f0f0f0; color:#888; }
...
uploaded_file = st.file_uploader("Upload", type=["csv","tsv","txt"],
...
if load_example or uploaded_file is not None:
    if load_example:
        ...
        genes = {
            ...
            "ACTB":   [7.9+np.random.normal(0,.3) for _ in sex],
        }
        df = pd.DataFrame({"sample_id": range(n), "sex": sex, **genes})
        fname = "example_rnaseq_GSE123456.csv"
    ...
    with st.spinner("Running analysis..."):
        ...
        degs  = compute_real_degs(df, sex_col, top_n=8)
        pct   = compute_field_percentile(bd["fairness_score"])
        recs  = generate_dynamic_recommendations(ratio, flags, tissue, power)

    score_color = ("#C1440E" if bd["fairness_score"] < 41 else
                   "#BA7517" if bd["fairness_score"] < 71 else "#3B6D11")
    loses_sig = sum(1 for d in degs if d["verdict"] == "loses sig.")
    context_line = (
        f"Better than {pct['pct_worse']}% of published RNA-seq studies"
        if pct['pct_worse'] > 50
        else f"Worse than {pct['pct_better']}% of published RNA-seq studies"
    )
    ...
    <div class="file-bar">
      <div style="flex:1;">
        ...
        <div class="file-meta">RNA-seq · {organism.split()[0]} · n={ratio['total']} · {tissue} · RadhaOmics v1.1</div>
      </div>
    </div>
    <div class="score-block">
      ...
      <div style="padding-bottom:4px;">
        ...
        <div style="font-size:11px;color:#888;margin-top:5px;">{context_line}</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with dc1:
        fig = go.Figure(go.Pie(
            labels=[f"Male ({ratio['male_pct']}%)",
                    f"Female ({ratio['female_pct']}%)"],
    ...
    with dc2:
        for label, ok, val, dot_color in [
            ("Sex ratio ≤ 1.5:1",  not ratio["ratio_flag"],
             f"{ratio['mf_ratio']}:1", "#E24B4A"),
            ("FDA SABV",           not ratio["ratio_flag"],
             "Fail" if ratio["ratio_flag"] else "Pass", "#E24B4A"),
            ("NIH SABV",           not ratio["ratio_flag"],
             "Fail" if ratio["ratio_flag"] else "Pass", "#E24B4A"),
            ("Female power ≥ 80%", not power["power_flag"],
             f"{round(power['achieved_power']*100)}%", "#EF9F27"),
            ("Sex metadata",       True, "Pass", "#639922"),
            ("Sample size ≥ 100",  ratio["total"] >= 100,
             str(ratio["total"]), "#639922"),
        ]:
            vc = "#3B6D11" if ok else "#C1440E"
            dc_color = "#639922" if ok else dot_color
            st.markdown(
                f'<div class="check-row">'
                f'<span><span class="dot" style="background:{dc_color};"></span>{label}</span>'
                f'<span style="color:{vc};font-weight:500;">{val}</span>'
                f'</div>',
                unsafe_allow_html=True
            )

    if degs:
        st.markdown('<span class="section-label">Differential expression impact — computed from your data</span>', unsafe_allow_html=True)
        rows = ""
        for d in degs:
            p_b_fmt   = fmt_p(d["p_biased"])
            p_bal_fmt = fmt_p(d["p_balanced"])
            p_b_cls   = "ps" if d["p_biased"] < 0.05 else "cn"
            p_bal_cls = "pn" if d["p_balanced"] >= 0.05 else "ps"
            v_cls     = "cp" if d["verdict"] == "loses sig." else ("cs" if d["verdict"] == "stable" else "cn")
            chr_disp  = d["chr"] if d["chr"] != "auto" else "—"
            rows += (
                f"<tr>"
                f"<td><span class='gn'>{d['gene']}</span></td>"
                f"<td style='color:#aaa;'>chr{chr_disp}</td>"
                f"<td><span class='{p_b_cls}'>{p_b_fmt}</span></td>"
                f"<td><span class='{p_bal_cls}'>{p_bal_fmt}</span></td>"
                f"<td style='font-family:monospace;font-size:10px;'>{'+' if d['fc']>0 else ''}{d['fc']}</td>"
                f"<td><span class='{v_cls}'>{d['verdict']}</span></td>"
                f"</tr>"
            )
        st.markdown(f"""
        <table class="deg-table">
          <thead><tr><th>Gene</th><th>Chr</th><th>p (biased)</th><th>p (balanced)</th><th>FC (log2)</th><th>Verdict</th></tr></thead>
          <tbody>{rows}</tbody>
        </table>
        <div style="font-size:10px;color:#aaa;margin-bottom:1rem;">{loses_sig} of {len(degs)} DEGs would lose significance in a sex-balanced cohort</div>
        """, unsafe_allow_html=True)
    ...
    <div style="font-size:11px;color:#555;margin-top:6px;padding-top:6px;border-top:0.5px solid #e5e7eb;">
      BiasΔ = ({imb} × {tw}) + {fp} = <strong style="color:{score_color};">{bd['biasdelta']}</strong>
    </div>
    """, unsafe_allow_html=True)
    ...
    for rec in recs:
        st.markdown(
            ...
            f'<div class="rec-num">{rec["num"]}</div>'
            f'<div class="rec-text"><strong>{rec["title"]}</strong> — {rec["detail"]}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    ...
    citation = (
        f"Sex bias analysis was performed using RadhaOmics v1.1 "
        ...
        f"{loses_sig} of {len(degs)} top DEGs lose significance in a sex-balanced cohort. "
    ...
    features = [
        ...
        ("DEG impact table",   "Real t-tests from your uploaded data"),
